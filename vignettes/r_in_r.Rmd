...
Think of the "standard" or baseline behavior of R as a baseline which
...
operational logic here, but let namesspacing behave normally.
...
normally keeps track of scope! So if you want to extend syntax, you end
up breaking scoping in the bargain.

This principle also suggests a test to make sure we are on the right
...
An attempt to model the S3/S4 dispatch, condition system, etc, are not made.
...
Things learned.
* Nomenclature.
  * Functions ending in `_` work on quotations and args, while
    corresponding to functions that work by NSE.
    * force(x) -> force_(arg(x))
    * () -> value_(arg(x))
    * `function` -> function_
  * Functions that operate on quos and args that don't end in `_`:
    * forced()
* Need a way to construct a forced promise.
* Decent way to emulate <- - unwrapping would be good too.
...
builtins <- as.environment(list(
  ...
  `<<-` = function(lval, rval) {
    ...
    munge_assignment(lval, rval,
                     then = function(lval, rval) {
                       where <- find(lval, parent.env(env(lval)))
                       assign(expr(lval), value_(rval), envir = env(lval))
...
Having cleared our throat, now it's time to consider how to tackle 'eval'.
